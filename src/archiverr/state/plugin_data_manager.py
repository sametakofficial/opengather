...
class PluginDataManager:
    ...
    def save_plugin_data(
        ...
        Args:
            ...
            get_job_func: Function to get job by ID
            run_id: Run ID for the plugin document
        """
        job = get_job_func(job_id) if get_job_func else None
        ...
        plugin_doc = {
            ...
            "plugin_name": plugin_name,
            "data": data,
            "run_id": job.run_id if job else run_id,
    ...
    def update_plugin_result(
        ...
        Args:
            job_index: Job index
            plugin_name: Plugin name
            result: Plugin result (PluginResult or dict)
            get_job_func: Function to get job by index
            run_id: Run ID
        """
        job = get_job_func(job_index) if get_job_func else None
        if not job:
        ...
        # Persist via delegate
        result_data = result.to_dict() if hasattr(result, 'to_dict') else data
