...
def denormalize_config(normalized: dict[str, Any], format: str = 'auto') -> dict[str, Any]:
    ...
    Useful for saving config back to file.
    
    Args:
        normalized: Normalized config with _plugins
        format: 'flexget', 'legacy', or 'auto' (use original format)
    
    Returns:
        Config in specified format (without internal keys)
    """
    if format == 'auto':
    ...
    if format == 'legacy':
        ...
        for name, conf in plugins.items():
            enabled = conf.get('_enabled', True)
            clean_conf = {k: v for k, v in conf.items() if not k.startswith('_')}
            if not enabled:
                if not clean_conf:
                    result['plugins'][name] = False
                else:
                    result['plugins'][name] = {**clean_conf, 'enabled': False}
            else:
                if clean_conf:
                    result['plugins'][name] = {**clean_conf, 'enabled': True}
                else:
                    result['plugins'][name] = {'enabled': True}
    else:
        # FlexGet format - plugins at top level
        for name, conf in plugins.items():
            enabled = conf.get('_enabled', True)
            clean_conf = {k: v for k, v in conf.items() if not k.startswith('_')}
            if not enabled:
