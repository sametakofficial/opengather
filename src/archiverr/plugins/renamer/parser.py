...
CLASSIC_PATTERNS: list[re.Pattern] = [
    re.compile(r"(?i)^(?P<name>.+?)[\.\s]S(?P<season>\d{1,2})E(?P<episode>\d{1,3})"),
    re.compile(r"(?i)^(?P<name>.+?)[\.\s](?P<season>\d{1,2})x(?P<episode>\d{1,3})"),
...
def sanitize_string(s: str, custom_delete_keywords: list[str] | None = None) -> str:
    """
...
def _classic_try(s: str) -> tuple[str, int, int] | None:
...
def _wide_try(s: str) -> tuple[int | None, int | None]:
...
def parse_movie_name(
    ...
    custom_delete_keywords: list[str] | None = None
) -> tuple[str, int | None]:
    """
