...
class PluginDiscovery:
    ...
    Supports Jinja2 templates in manifest files.
    ...
    def discover(self) -> dict[str, dict[str, Any]]:
        ...
        for plugin_dir in self.plugins_dir.iterdir():
            ...
            self.debugger.debug("discovery", "Found plugin",
                              name=plugin_name, category=category,
                              version=version, manifest=manifest_type)

        return plugins

    def _load_plugin_metadata(self, plugin_dir: Path) -> dict[str, Any] | None:
        """
            ...
            for yaml_file in [manifest_yml, manifest_yaml, plugin_yml, plugin_yaml]:
                if yaml_file.exists():
                    try:
                        ...
                        self.debugger.error("discovery", f"Unexpected error loading {yaml_file.name}",
                                          dir=plugin_dir.name, error=str(e))
                        continue
        ...
        if raw_data is None and plugin_json.exists():
            try:
                ...
                self.debugger.error("discovery", "Unexpected error loading plugin.json",
                                  dir=plugin_dir.name, error=str(e))
